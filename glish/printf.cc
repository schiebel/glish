...
#define	PROGNAME	"GLISH"

static char	*ctor(int,int);
static void	doit( char*, char*, const_args_list*, int&, int, int, int, int);
static void	ctrl( char * );
static int	digit( char *, int );
static void	illfmt( char*, char*, int, char* );

void gprintf( char *format, const_args_list *args, int elemlen )
{
...
cvt:
		...
		case 'c': case 's':
			if (flags)
				illfmt(cbuf, convp, ch, hasmod);
			ap = doit(cbuf, convp, args, index, i, ndyn, ch, ch);
			goto scan;

		/* integer formats */
			case 'd': case 'i': case 'o': case 'u': case 'x': case 'X':
			if ((flags & (LONG|SHORT)) == (LONG|SHORT))
				illfmt(cbuf, convp, ch, "is both long and short");
			ap = doit(cbuf, convp, args, index, i, ndyn, ch,
				flags & LONG ? 'l' : flags & SHORT ? 'h' : 'i');
			goto scan;
		...
		case 'e': case 'E': case 'f': case 'g': case 'G':
			if (flags)
				illfmt(cbuf, convp, ch, hasmod);
			ap = doit(cbuf, convp, args, index, i, ndyn, ch, 'f');
			goto scan;
		...
		case 'r': case 'R':
			if (flags)
				illfmt(cbuf, convp, ch, hasmod);
			ap = doit(cbuf, convp, args, index, i, ndyn, 's', ch);
			goto scan;
		...
		default:	/* something we cannot handle */
			if (isascii(ch) && isprint(ch))
				...
				(void) sprintf(cbuf, "\\%03o", (unsigned char)ch);
			(void) fprintf(stderr,
				"%s: illegal conversion character `%s'\n",
				PROGNAME, cbuf);
			exit(EX_USAGE);
			/* NOTREACHED */
		}
		...
		if (convp >= &cbuf[sizeof(cbuf) - 2]) {
			(void) fprintf(stderr, "%s: conversion string too long\n",
				PROGNAME);
			exit(EX_USAGE);
		}
		*convp++ = ch;
...
static void illfmt( char *cbuf, char *convp, int ch, char *why ) {
	*convp++ = ch;
	*convp = 0;
	(void) fprintf(stderr, "%s: format `%s' illegal: %s\n",
		PROGNAME, cbuf, why);
	exit(EX_USAGE);
}
...
#define STRING(lhs,strdup)						\
{									\
	val = (*ap)[index];						\
	if ( val->Type() != TYPE_STRING )				\
		{							\
		(void) fprintf(stderr,"bad type, non-string\n");	\
		exit(EX_USAGE);						\
		}							\
	lhs = strdup(val->StringPtr(0)[off]);				\
	++index;							\
}
#define INT(lhs)							\
{									\
	...
	if ( ! val->IsNumeric() )					\
		{							\
		(void) fprintf(stderr,"bad type, non-numeric\n");	\
		exit(EX_USAGE);						\
		}							\
...
{									\
	...
	if ( ! val->IsNumeric() )					\
		{							\
		(void) fprintf(stderr,"bad type, non-float\n");		\
		exit(EX_USAGE);						\
		}							\
...
static void doit( char *cbuf, char *convp, const_args_list *ap, int &index, int off, int ndyn, int cch, int cty)
{
	register char *s;
	const IValue *val;
	...
	*s = 0;
	...
	if (index >= (*ap)[index]->Length()) {
		(void) fprintf(stderr,
			"%s: not enough args for format `%s'\n",
			PROGNAME, s);
		exit(EX_USAGE);
	}
